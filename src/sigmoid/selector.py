...
class Selector:
    ...
    def __init__(self, data: np.ndarray, seed: int | None = None) -> None:
        ...
        Args:
            data: Binary data matrix of shape (samples, features).
            seed: Random seed for reproducibility. Defaults to None.
        """
        self.raw = data
        self.optimal: Model | None = None
    ...
    def fit(
        ...
        Args:
            ...
            its: Number of iterations for training each model.
            repeats: Number of repeats per latent dimension to ensure robustness.
        ...
        for i, latent_dim in enumerate(k_range):
            for j in range(repeats):
                ...
                seed = int(init_seeds[i, j])

                if verbose:
                    logger.info(
                        "Training candidate %d/%d: latent_dim=%d repeat=%d/%d seed=%d",
                        done,
                        total,
                        latent_dim,
                        j + 1,
    ...
    def trace_df(self):
        ...
        Lazy import so pandas remains an optional convenience, not a hard dep.
        """
        try:
            import pandas as pd
